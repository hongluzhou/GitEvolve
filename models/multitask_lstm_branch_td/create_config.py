...
def create_path_config():
    ...
    path_config = {
            ...
            'exp_save_dir': os.path.join(
                    root_dir, 'exp', 'www_get_statistics'),  # www_branch_td www_get_statistics
            ...
            # --- the directory to user created repo
            'user_created_repo_path': os.path.join(
                    root_dir, 'data',
                    'user_created_repo.json'),

            # --- the directory to repo_watch_count
            'repo_watch_count_path': os.path.join(
                    root_dir, 'data',
                    'user_activity_feat_no_normalization_' +
                    '2015-01-01_2017-07-31',
                    'repo_watch_count.json'),

            # --- the directory to repo_fork_count
            'repo_fork_count_path': os.path.join(
                    root_dir, 'data',
                    'user_activity_feat_no_normalization_' +
                    '2015-01-01_2017-07-31',
                    'repo_fork_count.json'),
...
def create_dataset_config():
    dataset_config = {
            ...
            'use_user_cluster_one_hot': True,
            'use_user_cluster_minmax': False,
            'use_cluster_profile_features': False,
            'use_cluster_activity_features': True,
            ...
            'dim_user_profile_features': 9,
            'dim_user_activity_features': 28,  # 2 + 2 + 12 * 2
            ...
            # (should be 14*2 only for cluster acitivity features, wrong here)
...
def create_testing_config():
    testing_config = {
            ...
            'keep_pred_max': 20,  # keep prediction threshold 20
            'keep_pred_round': 4
            }
...
def create_evaluation_config():
    config = {
            'event_type_nlg_eval': False,
            'event_type_map_eval': False,
            'event_type_categorical_accuracy_eval_given_gt': False,
            'event_type_percentage_eval': False,

            'time_delay_overall_evaluation': True,

            'user_cluster_nlg_eval': False,
            'user_cluster_map_eval': False,
            'user_cluster_categorical_accuracy_eval_given_gt': False,
