...
@author: zhouhonglu
"""
# from models.multitask_repeat_last_action.utils import categorical_focal_loss
# from models.multitask_repeat_last_action.utils import weighted_et_bce
# from models.multitask_repeat_last_action.utils import weighted_uc_bce
...
from models.multitask_repeat_last_action.utils import time_delay_normalization
from models.multitask_repeat_last_action.utils import (
        time_delay_normalization_reverse)
from models.multitask_repeat_last_action.utils import load_jsongz_2_dataframe
from models.multitask_repeat_last_action.utils import (
        insert_no_event_for_a_chain_new)
from models.multitask_repeat_last_action.utils import (
        insert_no_event_for_a_sim_GTchain)
from models.multitask_repeat_last_action.utils import get_time_delay
from models.multitask_repeat_last_action.utils import utc_timestamp
from models.multitask_repeat_last_action.utils import (
...
def testset_creation(config, GT_avail=True):
    ...
    for repo in repo_list:  # for each cascade chain
        ...
        last_et = input_event_type[-1]
        last_td = input_time_delay[-1]
        last_uc = input_cluster[-1]
        if last_td == config['empty_time_delay']:
            last_td = np.random.uniform(low=lowtd_normalized, high=hightd_normalized)
        if last_td == 0.0:
            last_td = np.random.uniform(low=lowtd_normalized, high=td_normalized2)
