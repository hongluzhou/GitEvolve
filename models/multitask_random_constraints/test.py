...
@author: zhouhonglu
"""
# from models.multitask_random_constraints.utils import categorical_focal_loss
# from models.multitask_random_constraints.utils import weighted_et_bce
# from models.multitask_random_constraints.utils import weighted_uc_bce
from models.multitask_random_constraints.utils import set_logger
from models.multitask_random_constraints.utils import print_and_log
from models.multitask_random_constraints.utils import time_delay_normalization
from models.multitask_random_constraints.utils import (
        time_delay_normalization_reverse)
from models.multitask_random_constraints.utils import load_jsongz_2_dataframe
from models.multitask_random_constraints.utils import (
        insert_no_event_for_a_chain_new)
from models.multitask_random_constraints.utils import (
        insert_no_event_for_a_sim_GTchain)
from models.multitask_random_constraints.utils import get_time_delay
from models.multitask_random_constraints.utils import utc_timestamp
from models.multitask_random_constraints.utils import (
...
def simulation(config, model, testset):
    ...
    eventtype_ids = list(config['eventtype_2_id'].values())
    ...
    for i in range(len(testset['X_test'])):
        ...
        [min_td, max_td, one_chain_cluster] = testset['X_test'][i]
        ...
        predict_event = random.choice(eventtype_ids)

        # modify time_delay and time_delay_hour, user cluster
        time_delay = np.random.uniform(low=min_td, high=max_td)
        ...
        predict_user_cluster = random.choice(one_chain_cluster)
...
def testset_creation(config, GT_avail=True):
    ...
    for repo in repo_list:  # for each cascade chain
        ...
        # turn one_chain_user into one_chain_cluster
        one_chain_cluster = []
        for user in one_chain_user:
            try:
                one_chain_cluster.append(user_clusters[user])
            except KeyError:
                user_has_no_cluster.add(user)
                one_chain_cluster.append(user_clusters['no_event_user'])

        # get the range of time delay values
        min_td = min(one_chain_time_delay)
        max_td = max(one_chain_time_delay)

        # get all users who have interacted with this repo
        one_chain_cluster = list(set(one_chain_cluster))
        ...
        if min_td == config['empty_time_delay']:
            min_td = np.random.uniform(low=lowtd_normalized, high=hightd_normalized)
        if min_td == 0.0:
            min_td = np.random.uniform(low=lowtd_normalized, high=td_normalized2)
        if max_td == config['empty_time_delay']:
            max_td = np.random.uniform(low=min_td, high=hightd_normalized)
        if max_td == 0.0:
            max_td = np.random.uniform(low=min_td, high=td_normalized2)

        # initialize input for this sample
        x_vec = [min_td, max_td, one_chain_cluster]
