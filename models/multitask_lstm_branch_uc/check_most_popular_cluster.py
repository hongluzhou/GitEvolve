...
def train_models(config):
    ...
    #                        time_delay_output,
    #                        user_cluster_output])
    model = Model(inputs=input_sequences, outputs=user_cluster_output)
    ...
    # train val generator
    training_generator = DataGenerator_UC(
    ...
    validation_generator = DataGenerator_UC(
            os.path.join(config['exp_save_dir'], "dataset"),
...
def trainset_valiset_creation(config):
    ...
    allclus = set(cluster_act_feat.keys())
    clus_pop = dict()
    for clus in allclus:
        clus_pop[clus] = 0
    clus_pop[100] = 0
...
#            pdb.set_trace()
    ...
    most_active = 0
    most_active_clus = None
    for clus in clus_pop:
        if clus == 100:
            continue
        if clus_pop[clus] > most_active:
            most_active_clus = clus
            most_active = clus_pop[clus]
    pdb.set_trace()
...
class DataGenerator_UC(keras.utils.Sequence):
    """
    Reference:
    ...
    def __init__(self, data_path, list_IDs, labels,
                 batch_size, dim, window_size, uc_classes,
                 shuffle=False):
        'Initialization'
