...
@author: zhouhonglu
"""
from models.multitask_lstm_branch_uc.utils import categorical_focal_loss
from models.multitask_lstm_branch_uc.utils import weighted_et_bce
from models.multitask_lstm_branch_uc.utils import weighted_uc_bce
from models.multitask_lstm_branch_uc.utils import set_logger
from models.multitask_lstm_branch_uc.utils import print_and_log
from models.multitask_lstm_branch_uc.utils import time_delay_normalization
from models.multitask_lstm_branch_uc.utils import (
        time_delay_normalization_reverse)
from models.multitask_lstm_branch_uc.utils import load_jsongz_2_dataframe
from models.multitask_lstm_branch_uc.utils import (
        insert_no_event_for_a_chain_new)
from models.multitask_lstm_branch_uc.utils import (
        insert_no_event_for_a_sim_GTchain)
from models.multitask_lstm_branch_uc.utils import get_time_delay
from models.multitask_lstm_branch_uc.utils import utc_timestamp
from models.multitask_lstm_branch_uc.utils import (
...
def test(config, process_create_dataset=True):
    if process_create_dataset:
        ...
        if config['given_gt']:
            ...
            testset = testset_creation(config)  # not implemented
    else:
        # load testset
    ...
    if config['et_loss'] == 'focalloss':
        ...
        custom_object = {
                'weighted_et_bce': weighted_et_bce,
                'weighted_uc_bce': weighted_uc_bce
                }
    ...
    if not os.path.exists(trained_model_path):
        if config['load_model_epoch'] < 10:
            trained_model_path = os.path.join(
                config['exp_save_dir'], "models",
                'model-0{}.hdf5'.format(config['load_model_epoch']))
        else:
...
def simulation(config, model, testset):
    ...
    for i in range(len(testset['X_test'])):
        ...
        if predict_event == eventtype_2_id['no_event_for_1month'] or (
                time_delay_hour >= float(30*24)) or (
                        np.argmax(user_cluster[0]) == user_clusters[
                                'no_event_user']):
        ...
        if predict_user_cluster > empty_no_event_user_cluster:
            predict_user_cluster = empty_no_event_user_cluster
...
def testset_creation_given_gt(config, GT_avail=True):
    ...
    if config['use_user_profile_features']:
        ...
        with open(config['user_profile_feat_path'], 'r') as f:
            user_profile_feat = json.load(f)
        with open(config["user_created_repo_path"], 'rb') as f:
            user_created_repo = json.load(f)
        with open(config["repo_watch_count_path"], 'rb') as f:
            repo_watch_count = json.load(f)
        with open(config["repo_fork_count_path"], 'rb') as f:
            repo_fork_count = json.load(f)

        for user in user_profile_feat:
            this_user_feat = []
            user_profile_feat[user]['geolocation'][0] = \
                time_delay_normalization(
                            user_profile_feat[user]['geolocation'][0],
                            config['time_delay_normalization_func'])
            user_profile_feat[user]['ghimpact'][0] = \
                time_delay_normalization(
                            user_profile_feat[user]['ghimpact'][0],
                            config['time_delay_normalization_func'])
            user_profile_feat[user]['no_followers'][0] = \
                time_delay_normalization(
                            user_profile_feat[user]['no_followers'][0],
                            config['time_delay_normalization_func'])
            user_profile_feat[user]['no_following'][0] = \
                time_delay_normalization(
                            user_profile_feat[user]['no_following'][0],
                            config['time_delay_normalization_func'])

            try:
                user_profile_feat[user]['no_created_repo'] = [
                        len(user_created_repo[user])]
            except KeyError:
                user_profile_feat[user]['no_created_repo'] = [0]
            user_profile_feat[user]['avg_watch_created_repo'] = list()
            user_profile_feat[user]['avg_fork_created_repo'] = list()
            no_watch_created_repo = list()
            no_fork_created_repo = list()
            if user_profile_feat[user]['no_created_repo'][0] > 0:
                for repo in user_created_repo[user]:
                    try:
                        no_watch_created_repo.append(repo_watch_count[repo])
                    except KeyError:
                        no_watch_created_repo.append(0)
                    try:
                        no_fork_created_repo.append(repo_fork_count[repo])
                    except KeyError:
                        no_fork_created_repo.append(0)
            try:
                user_profile_feat[user]["avg_watch_created_repo"].append(
                        np.average(no_watch_created_repo))
            except IndexError:
                user_profile_feat[user]["avg_watch_created_repo"].append(0)
            try:
                user_profile_feat[user]["avg_fork_created_repo"].append(
                        np.average(no_fork_created_repo))
            except IndexError:
                user_profile_feat[user]["avg_fork_created_repo"].append(0)

            user_profile_feat[user]['no_created_repo'][0] = \
                time_delay_normalization(
                            user_profile_feat[user][
                                    'no_created_repo'][0],
                            config['time_delay_normalization_func'])
            user_profile_feat[user]['avg_watch_created_repo'][0] = \
                time_delay_normalization(
                            user_profile_feat[user][
                                    'avg_watch_created_repo'][0],
                            config['time_delay_normalization_func'])
            user_profile_feat[user]['avg_fork_created_repo'][0] = \
                time_delay_normalization(
                            user_profile_feat[user][
                                    'avg_fork_created_repo'][0],
                            config['time_delay_normalization_func'])
            for feat_name in user_profile_feat[user]:
                this_user_feat += user_profile_feat[user][feat_name]
            user_profile_feat[user] = this_user_feat
            if sum(this_user_feat) > max_sum_profile_feat:
                max_sum_profile_feat = sum(this_user_feat)
                max_sum_profile_feat_vec = this_user_feat

        user_profile_feat["no_event_user"] = [0] * config[
                'dim_user_profile_features']
        user_profile_feat[config["empty_user"]] = [0] * config[
                'dim_user_profile_features']
        print_and_log(logger, "max_sum_profile_feat: {}".format(
                max_sum_profile_feat))
        print_and_log(logger, "max_sum_profile_feat_vec: {}".format(
    ...
    if config['use_user_activity_features']:
        ...
        with open(config['user_activity_feat_path'], 'r') as f:
            user_act_feat = json.load(f)
        for user in user_act_feat:
            for repo in user_act_feat[user]:
                this_user_repo_feat = []
                if repo == 'all':
                    for i in range(len(config['eventtype_2_id'])):
                        user_act_feat[user][repo][
                                'total_count_per_et_all_repo'][i] = \
                            time_delay_normalization(
                                    user_act_feat[user][repo][
                                            'total_count_per_et_all_repo'][i],
                                    config['time_delay_normalization_func']
                                    )
                    this_user_repo_feat += user_act_feat[
                            user][repo]['total_count_per_et_all_repo']
                    this_user_repo_feat.append(
                            time_delay_normalization(
                                    user_act_feat[user][repo][
                                            'avg_td_hour_all_repo'],
                                    config['time_delay_normalization_func'])
                            )
                    this_user_repo_feat.append(
                            time_delay_normalization(
                                    user_act_feat[user][repo][
                                            'std_td_hour_all_repo'],
                                    config['time_delay_normalization_func'])
                            )
                else:
                    for i in range(len(config['eventtype_2_id'])):
                        user_act_feat[user][repo][
                                'total_count_per_et_this_repo'][i] = \
                            time_delay_normalization(
                                    user_act_feat[user][repo][
                                            'total_count_per_et_this_repo'][i],
                                    config['time_delay_normalization_func']
                    ...
                    this_user_repo_feat += user_act_feat[
                            user][repo]['total_count_per_et_this_repo']
                    this_user_repo_feat.append(
                            time_delay_normalization(
                                    user_act_feat[user][repo][
                                            'avg_td_hour_this_repo'],
                                    config['time_delay_normalization_func'])
                            )
                    this_user_repo_feat.append(
                            time_delay_normalization(
                                    user_act_feat[user][repo][
                                            'std_td_hour_this_repo'],
                                    config['time_delay_normalization_func'])
                            )
                user_act_feat[user][repo] = this_user_repo_feat
                if sum(this_user_repo_feat) > max_sum_act_feat:
                    max_sum_act_feat = sum(this_user_repo_feat)
                    max_sum_act_feat_vec = this_user_repo_feat
        print_and_log(logger, "max_sum_act_feat: {}".format(
                max_sum_act_feat))
        print_and_log(logger, "max_sum_act_feat_vec: {}".format(
                max_sum_act_feat_vec))
...
def simulation_given_gt(config, model, testset):
    ...
    yhat_user_cluster = model.predict(testset['X_test'], verbose=0)
